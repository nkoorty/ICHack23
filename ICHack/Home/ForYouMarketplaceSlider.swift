...
class ForYouMarketPlaceSlider: UIView {
    ...
    lazy var foryouButton: UIButton = {
        let button = UIButton()
        button.layout(textcolour: .black, backgroundColour: .clear, size: 16, text: "Your Health", image: nil, cornerRadius: 0)
        return button
    }()
    
    lazy var underlineView: UIView = {
        let view = UIView()
        view.backgroundColor = .icGreen
        return view
    }()
    
    lazy var marketplaceButton: UIButton = {
        let button = UIButton()
        button.layout(textcolour: .black, backgroundColour: .clear, size: 16, text: "History", image: nil, cornerRadius: 0)
        return button
    }()
